  ...
  switch (action.type) {
    ...
    case BOOK_SAVED:
      return Object.assign({}, state, {
        booksJustSaved: [{ ...action.book, success: true }].concat(state.booksJustSaved.slice(0, MAX_BOOKS_DISPLAYED)),
        pendingNumber: state.pendingNumber - 1 || 0
      });
    case BOOK_LOOKUP_FAILED:
      let entry = { _id: "" + new Date(), title: `Failed lookup for ${action.isbn}`, success: false };
      return Object.assign({}, state, {
        booksJustSaved: [entry].concat(state.booksJustSaved.slice(0, MAX_BOOKS_DISPLAYED)),
        pendingNumber: state.pendingNumber - 1 || 0
      });
  }
  return state;
