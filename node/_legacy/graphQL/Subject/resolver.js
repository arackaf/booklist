...
import SubjectMetadata from "./Subject.js";
    ...
    async allSubjects(root, args, context, ast) {
      ...
      if (queryPacket.metadataRequested.size) {
        ...
        if (queryPacket.metadataRequested.get("count")) {
          ...
          let countResults = await dbHelpers.runQuery(db, "subjects", [$match, { $group: { _id: null, count: { $sum: 1 } } }]);
  ...
  Mutation: {
    async createSubject(root, args, context, ast) {
      ...
      return await resolverHelpers.runMutation(session, transaction, async () => {
        ...
        let result = $project
          ? (await loadSubjects(db, [{ $match: { _id: newObject._id } }, { $project }, { $limit: 1 }], root, args, context, ast))[0]
          : null;
