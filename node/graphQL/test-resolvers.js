...
const runQueries = async () => {
  await processQuery(
    `{allBooks(LIMIT:1){Books{_id ean isbn title smallImage mediumImage userId publisher publicationDate pages authors subjects tags isRead dateAdded editorialReviews {source,content} similarItems similarItemsLastUpdate timestamp}}}`,
    "Book",
    "_id ean isbn title smallImage mediumImage userId publisher publicationDate pages authors subjects tags isRead dateAdded editorialReviews {source,content} similarItems similarItemsLastUpdate timestamp"
