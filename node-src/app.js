...
var dao = require('./dataAccess/dao').default;
...
var bookEntryQueueManager = require('./app-helpers/bookEntryQueueManager').default;
var PendingBookEntryDao = require('./dataAccess/pendingBookEntryDAO').default;
var ErrorLoggerDao = require('./dataAccess/errorLoggerDAO').default;
var UserDao = require('./dataAccess/userDAO').default;
...
var multerBookCoverUploadStorage = multer.diskStorage({
...
var upload = multer({ storage: multerBookCoverUploadStorage });
...
app.post('/react-redux/upload', upload.single('fileUploaded'), function (req, response) {
    ...
    try {
        lwip.open(pathToFileUploaded, function (err, image) {
            ...
            if (ext == '.jpg' || ext == '.jpeg') {
                fs.readFile(pathToFileUploaded, function (err, data) {
                    if (err) {
                    ...
                    if (batchImage) {
                        batchImage.exec(function (err, image) {
                            return processImageAsNeeded(image);
                        });
                    } else {
    ...
    function processImageAsNeeded(image) {
        if (image.width() > 55) {
            ...
            image.resize(50, newWidth, function (err, image) {
                ...
                image.writeFile(resizedDestination, function (err) {
...
app.post('/react-redux/createUser', function (req, response) {
    ...
    userDao.checkUserExists(username, password).then(function (exists) {
        if (exists) {
            ...
            userDao.createUser(username, password, rememberMe).then(function () {
...
app.get('/activate/:code', function (req, response) {
    ...
    userDao.activateUser(code).then(function (result) {
...
Promise.resolve(dao.init()).then(function () {
