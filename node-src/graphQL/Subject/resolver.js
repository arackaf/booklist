...
export default {
  ...
  Mutation: {
    ...
    async updateSubject(root, args, context, ast) {
      ...
      let res = await processHook(hooksObj, "Subject", "beforeUpdate", $match, updates, root, args, context, ast);
      if (res === false){
        return { Subject: null };
      }
      if (updates.$set || updates.$inc || updates.$push || updates.$pull || updates.$addToSet) {
        await db.collection("subjects").update($match, updates);
      }
    ...
    async updateSubjects(root, args, context, ast) {
      ...
      if (updates.$set || updates.$inc || updates.$push || updates.$pull || updates.$addToSet) {
        await db.collection("subjects").update($match, updates, { multi: true });
      }
      await processHook(hooksObj, "Subject", "afterUpdate", $match, updates, root, args, context, ast);
    ...
    async deleteSubject(root, args, context, ast) {
      ...
      let res = await processHook(hooksObj, "Subject", "beforeDelete", $match, root, args, context, ast);
      if (res === false) {
        return false;
      }
      await db.collection("subjects").remove($match);
