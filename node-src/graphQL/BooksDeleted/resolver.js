...
export async function loadBooksDeleteds(db, queryPacket, root, args, context, ast) {
  ...
  await processHook(hooksObj, "BooksDeleted", "queryPreAggregate", aggregateItems, root, args, context, ast);
...
export default {
  Query: {
    async getBooksDeleted(root, args, context, ast) {
      await processHook(hooksObj, "BooksDeleted", "queryPreprocess", root, args, context, ast);
      let db = await (typeof root.db === "function" ? root.db() : root.db);
      context.__mongodb = db;
      let queryPacket = decontructGraphqlQuery(args, ast, BooksDeletedMetadata, "BooksDeleted");
      await processHook(hooksObj, "BooksDeleted", "queryMiddleware", queryPacket, root, args, context, ast);
    ...
    async allBooksDeleteds(root, args, context, ast) {
      ...
      if (queryPacket.metadataRequested.size) {
        ...
        if (queryPacket.metadataRequested.get("count")) {
          let countResults = await dbHelpers.runQuery(db, "booksDeleted", [{ $match: queryPacket.$match }, { $group: { _id: null, count: { $sum: 1 } } }]);  
