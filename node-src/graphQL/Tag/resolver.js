...
export default {
  ...
  Mutation: {
    async createTag(root, args, context, ast) {
      ...
      let { db, session, transaction } = await resolverHelpers.startDbMutation(gqlPacket, "Tag", TagMetadata, { create: true });
      return await resolverHelpers.runMutation(session, transaction, async() => {
        ...
        newObject = await dbHelpers.processInsertion(db, newObject, { ...gqlPacket, typeMetadata: TagMetadata, session });
        if (newObject == null) {
          return { Tag: null };
        }
    ...
    async updateTag(root, args, context, ast) {
      let gqlPacket = { root, args, context, ast, hooksObj };
      let { db, session, transaction } = await resolverHelpers.startDbMutation(gqlPacket, "Tag", TagMetadata, { update: true });
      return await resolverHelpers.runMutation(session, transaction, async() => {
    ...
    async deleteTag(root, args, context, ast) {
      ...
      let gqlPacket = { root, args, context, ast, hooksObj };
      let { db, session, transaction } = await resolverHelpers.startDbMutation(gqlPacket, "Tag", TagMetadata, { delete: true });
      try {
